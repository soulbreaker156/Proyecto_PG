        ...
        <div>
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col">
                    ...
                    <input className="border" type="number" name="monto" value={data.monto ?? ''} onChange={(e) => setData('monto', e.target.value === '' ? null : Number(e.target.value))} />
                    {errors.monto && <span className="text-red-500">{errors.monto}</span>}
                </div>
                <div className="flex flex-col">
                    <label htmlFor="descripcion">Descripción</label>
                    <textarea className="border" name="descripcion" value={data.descripcion} onChange={(e) => setData('descripcion', e.target.value)}></textarea>
                    {errors.descripcion && <span className="text-red-500">{errors.descripcion}</span>}
                </div>
                <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" type="submit">Guardar Crédito</button>
            </form>
        </div>
    );
}